...
- `pytest tests/` is green and coverage stays at or above the floor in
  ...
  same config value. Run `make cov` for the current number; it is deliberately not
  written down here, because a percentage in prose goes stale the moment someone
  commits a file. Closing a coverage gap is the work; editing the number down is
  not.
