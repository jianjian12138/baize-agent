...
5. 在需要时补充调用 `picasso-dev-methods`，收敛方法论执行方式
...
无论是单运行端、Claude Code + Codex 协作，还是 OpenClaw / 其他平台模式，默认放行结论都不是 `READY`，而是 `NEEDS_WORK`。
